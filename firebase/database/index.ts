...
export const createCollectionAgencia = async (name: string, database: Firestore) => {
  try {
    ...
    await setDoc(docRef, { contacto: "+12224343434", politicaPrivacidad: "https://example.com/polity", AI: true });
    console.log("Colección " + name + " creada.");
    console.log("Agencia creada con número de contacto: +12224343434, URL de politica de privacidad https://example.com/polity y AI en true");
  } catch (error) {
    console.error("Error creando la colección " + name, error);
  }
};